...
final class ResolverException extends ContainerException
{
	...
	public static function factoryNotResolvable(DefinitionInterface $definition): self
	{
		throw new self(
			sprintf(
				'Failed to resolve definition "%s": The factory class must implement the "__invoke()" method.',
				$definition->getName(),
			),
		);
	}
